  ...
  App.vue.interfaces.contentFormatters = {
    methods: {
      _formatToCurrency: function(value, custom_number_format, nullReturn) {
        ...
        return numeral(value).format('$' + (custom_number_format || App.vue.config.contentFormatters.currency));
      },
      _formatToPercent: function(value, custom_number_format, nullReturn) {
        if (value === undefined || value === null || isNaN(value)) {
         return nullReturn || App.vue.config.contentFormatters.empty; 
        }
        return numeral(value / 100).format((custom_number_format || App.vue.config.contentFormatters.percent) + '%');
      },
      _formatToNumber: function(value, custom_number_format, nullReturn) {
        if (value === undefined || value === null || isNaN(value)) {
         return nullReturn || App.vue.config.contentFormatters.empty; 
        }
        return numeral(value).format(custom_number_format || App.vue.config.contentFormatters.number);
      },
      ...
      _formatToDate: function(value, custom_format, nullReturn) {
        ...
        return moment(value).format(custom_format || App.vue.config.contentFormatters.date);
      }
    }
  };
})();
